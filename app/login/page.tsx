      ...
      <div className="w-full max-w-md">
        ...
        <div className="border border-gray-100 rounded-2xl p-8">
          ...
          <div className="mb-6">
            ...
            <input type="password" placeholder="Your password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-black"/>
            <a href="#" className="text-sm text-gray-400 hover:text-black mt-2 block text-right">Forgot password?</a>
          </div>
          {message && (
            <div className="mb-4 p-3 rounded-xl bg-gray-50 text-sm text-gray-700">{message}</div>
          )}
          <button onClick={handleLogin} disabled={loading} className="w-full bg-black text-white py-4 rounded-xl font-medium hover:bg-gray-800 transition-colors">
